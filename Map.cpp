...
Map::Map(int width, int height)
{
	...
	mapTreshold = Coordinate((this->width - scrWidth) / 2, (this->height - scrHeight) / 2);
...
{
	Coordinate centerObjPos = Coordinate(object->position.x + object->width / 2, object->position.y + object->height / 2);
...
{
	Coordinate centerObjectPosition = Coordinate(object->position.x + object->width / 2, object->position.y + object->height / 2);
	Coordinate center_ObjectPosition = Coordinate(_object->position.x + _object->width / 2, _object->position.y + _object->height / 2);

	Coordinate distance = centerObjectPosition.sub(center_ObjectPosition);
...
Coordinate Map::getAsteroidPosition()
{
	Coordinate position = Coordinate(rand() % width + playerTreshold.x + player->position.x, rand() % height + playerTreshold.y + player->position.y);
